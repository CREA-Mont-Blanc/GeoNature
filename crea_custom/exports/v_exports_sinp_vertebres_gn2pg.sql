    ...
    af_actors AS (
        SELECT
            ...
            json_build_object(
                ...
                'identity',
                CASE
                    WHEN cafa.id_organism IS NOT NULL THEN json_build_object('organism_name', borg.nom_organisme)
                    WHEN cafa.id_role IS NOT NULL THEN json_build_object(
                        'first_name',
                        tro.nom_role,
                        'last_name',
                        tro.prenom_role
        ...
        FROM
            ...
            JOIN ref_nomenclatures.t_nomenclatures tn ON cafa.id_nomenclature_actor_role = tn.id_nomenclature
    ),
    af_territories AS (
        SELECT
        ...
        FROM
            ...
            LEFT JOIN ref_nomenclatures.t_nomenclatures ON caft.id_nomenclature_territory = t_nomenclatures.id_nomenclature
        GROUP BY
            caft.id_acquisition_framework
    ),
    af_objectives AS (
        ...
        FROM
            ...
            LEFT JOIN ref_nomenclatures.t_nomenclatures ON cafo.id_nomenclature_objectif = t_nomenclatures.id_nomenclature
        GROUP BY
            cafo.id_acquisition_framework
    ),
    af_voletsinp AS (
        ...
        FROM
            ...
            LEFT JOIN ref_nomenclatures.t_nomenclatures ON cafv.id_nomenclature_voletsinp = t_nomenclatures.id_nomenclature
        GROUP BY
            cafv.id_acquisition_framework
    ),
    af_publication AS (
        SELECT
            ...
            array_agg(
                DISTINCT jsonb_build_object(
                    ...
                    'url',
                    sinp_datatype_publications.publication_url
                )
            ) AS publications
        FROM
            gn_meta.cor_acquisition_framework_publication cafp
            LEFT JOIN gn_meta.sinp_datatype_publications ON cafp.id_publication = sinp_datatype_publications.id_publication
        GROUP BY
            cafp.id_acquisition_framework
    ),
    af AS (
        ...
        FROM
            ...
            LEFT JOIN gn_meta.t_acquisition_frameworks tafp ON tafp.id_acquisition_framework = taf.acquisition_framework_parent_id
            JOIN af_actors ON af_actors.id_acquisition_framework = taf.id_acquisition_framework
            LEFT JOIN ref_nomenclatures.t_nomenclatures ntl ON taf.id_nomenclature_territorial_level = ntl.id_nomenclature
            LEFT JOIN ref_nomenclatures.t_nomenclatures nft ON taf.id_nomenclature_financing_type = nft.id_nomenclature
    ...
    ds_actors AS (
        SELECT
            ...
            json_build_object(
                ...
                'identity',
                CASE
                    WHEN cda.id_organism IS NOT NULL THEN json_build_object('organism_name', borg.nom_organisme)
                    WHEN cda.id_role IS NOT NULL THEN json_build_object(
                        'first_name',
                        tro.nom_role,
                        'last_name',
                        tro.prenom_role
    ...
    ds_protocols AS (
        SELECT
            ...
            jsonb_build_object(
                ...
                sdp.protocol_url,
                'type',
                t_nomenclatures.cd_nomenclature
            ) AS protocols
        FROM
            gn_meta.cor_dataset_protocol cdp
            JOIN gn_meta.sinp_datatype_protocols sdp ON cdp.id_protocol = sdp.id_protocol
            LEFT JOIN ref_nomenclatures.t_nomenclatures ON sdp.id_nomenclature_protocol_type = t_nomenclatures.id_nomenclature
    ),
    ds AS (
        SELECT
            tds.id_dataset,
            ...
            jsonb_build_object(
                ...
                'territories',
                array_agg(
                    DISTINCT ref_nomenclatures.get_cd_nomenclature (cdt.id_nomenclature_territory)
                ),
                'actors',
                json_agg(ds_actors.json_data)
            ) AS dataset_data
        FROM
...
FROM
    ...
    LEFT JOIN ds ON ds.id_dataset = s.id_dataset
    LEFT JOIN af ON ds.id_acquisition_framework = af.id_acquisition_framework
